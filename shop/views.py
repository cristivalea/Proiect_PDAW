...
from .models import Cart  # presupunând că ai model pentru coș
...
def place_order(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            cart.items.all().delete()  # goliți coșul după comandă
            return redirect('order_confirmation', order_id=order.id)
    else:
        form = OrderForm()

    return render(request, 'shop/place_order.html', {'form': form, 'cart': cart})
